...
import { aggiungiRigaDistinta } from "@/lib/distinteScaricoRepository";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const riga = await aggiungiRigaDistinta({ distintaId: id, articoloId, quantita });
    void logOperation(session.name, "CREATE", "distinta_scarico", id, { articoloId, quantita });
    return NextResponse.json(riga);
  } catch (e) {
    console.error("[distinte-scarico/[id]/righe POST]", e);
