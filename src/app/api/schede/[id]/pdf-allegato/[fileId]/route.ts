...
import { removePdfAllegatoFromScheda, getSchedaById } from "@/lib/schedeRepository";
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string; fileId: string }> }) {
  ...
  try {
    await removePdfAllegatoFromScheda(id, fileId);
    void logOperation(session.name, "UPDATE", "scheda", id, { azione: "elimina_pdf_allegato", fileId });
