...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Nessuna soglia/riordino da monitorare per Vernici: uno scarico che porti sotto zero
  // sarebbe solo dato sporco fino al prossimo inventario, quindi si blocca subito.
  ...
  // Anche uno scarico preciso è comunque un segnale che la vernice si è mossa — resta "da
  // verificare" finché non arriva una conta fisica al prossimo inventario (2026-08-22).
  await segnalaMovimentoVernice(id);

  void logOperation(session.name, "UPDATE", "movimento_magazzino", id, { categoria: "vernici", tipo: "scarico", quantita, giacenzaRisultante });
