...
import { addFase } from "@/lib/cicliVerniciaturaRepository";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const ciclo = await addFase(id, { ordine: body.ordine, nomeFase: body.nomeFase ?? null, note: body.note ?? null });
    void logOperation(session.name, "UPDATE", "ciclo_verniciatura", id, { azione: "aggiunta_fase", ordine: body.ordine });
    return NextResponse.json(ciclo, { status: 201 });
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e);
    console.error("[POST /api/verniciatura/cicli/:id/fasi]", message);
