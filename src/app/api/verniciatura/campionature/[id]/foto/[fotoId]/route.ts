...
import { deleteFoto } from "@/lib/campionatureVerniciaturaRepository";
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string; fotoId: string }> }) {
  try {
    ...
    void logOperation(session.name, "DELETE", "campionatura_verniciatura", id, { azione: "rimozione_foto", fotoId });
    return NextResponse.json({ ok: true });
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e);
    console.error("[DELETE /api/verniciatura/campionature/:id/foto/:fotoId]", message);
