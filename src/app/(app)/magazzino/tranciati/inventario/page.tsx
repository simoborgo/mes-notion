...
import NuovoInventarioTranciatiForm from "@/components/NuovoInventarioTranciatiForm";

export const dynamic = "force-dynamic";

export default async function InventarioTranciatiPage() {
  const session = await getSession();
  if (!session) redirect("/login");
  if (!MAGAZZINO_TRANCIATI_ROLES.includes(session.role)) redirect("/");

  const sessione = await getInventarioAperto("tranciati");
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold" style={{ fontFamily: "var(--font-display)" }}>
            Inventario Tranciati
          </h1>
          <p className="text-sm mt-1" style={{ color: "var(--color-grey-mid)" }}>
            Conteggio a riconteggio delle scorte
          </p>
        </div>
        <Link href="/magazzino/tranciati" className="text-xs underline" style={{ color: "var(--color-primary)" }}>
          ← Magazzino
        </Link>
      </div>
...
async function InventarioAttivoCard({ inventarioId, apertoDa }: { inventarioId: string; apertoDa: string }) {
  ...
  return (
    <div className="rounded-xl border-2 p-4 space-y-3" style={{ borderColor: "#FCD34D", background: "#FFFBEB" }}>
      ...
      <Link
        href={`/magazzino/tranciati/inventario/${inventarioId}`}
