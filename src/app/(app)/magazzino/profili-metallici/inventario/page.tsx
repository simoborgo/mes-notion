...
import NuovoInventarioProfiliMetalliciForm from "@/components/NuovoInventarioProfiliMetalliciForm";

export const dynamic = "force-dynamic";

export default async function InventarioProfiliMetalliciPage() {
  const session = await getSession();
  if (!session) redirect("/login");
  if (!MAGAZZINO_PROFILI_METALLICI_ROLES.includes(session.role)) redirect("/");

  const sessione = await getInventarioAperto("profili_metallici");
  ...
  return (
    <div className="max-w-lg mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold" style={{ fontFamily: "var(--font-display)" }}>
            Inventario Profili Metallici
          </h1>
          <p className="text-sm mt-1" style={{ color: "var(--color-grey-mid)" }}>
            Conteggio a riconteggio delle scorte
          </p>
        </div>
        <Link href="/magazzino/profili-metallici" className="text-xs underline" style={{ color: "var(--color-primary)" }}>
          ← Magazzino
        </Link>
      </div>
      ...
      {sessione ? (
        ...
        <NuovoInventarioProfiliMetalliciForm />
      )}
    </div>
  );
}
...
async function InventarioAttivoCard({ inventarioId, apertoDa }: { inventarioId: string; apertoDa: string }) {
  ...
  return (
    <div className="rounded-xl border-2 p-4 space-y-3" style={{ borderColor: "#FCD34D", background: "#FFFBEB" }}>
      ...
      <Link
        href={`/magazzino/profili-metallici/inventario/${inventarioId}`}
