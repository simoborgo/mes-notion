...
import NuovoInventarioBordiForm from "@/components/NuovoInventarioBordiForm";

export const dynamic = "force-dynamic";

export default async function InventarioBordiPage() {
  const session = await getSession();
  if (!session) redirect("/login");
  if (!MAGAZZINO_BORDI_ROLES.includes(session.role)) redirect("/");

  const sessione = await getInventarioAperto("bordi");
  ...
  return (
    <div className="max-w-lg mx-auto space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/magazzino/bordi" className="text-xs underline" style={{ color: "var(--color-primary)" }}>
          ← Magazzino
        </Link>
      </div>
...
async function InventarioAttivoCard({ inventarioId, apertoDa }: { inventarioId: string; apertoDa: string }) {
  ...
  return (
    <div className="rounded-xl border-2 p-4 space-y-3" style={{ borderColor: "#FCD34D", background: "#FFFBEB" }}>
      ...
      <Link
        href={`/magazzino/bordi/inventario/${inventarioId}`}
