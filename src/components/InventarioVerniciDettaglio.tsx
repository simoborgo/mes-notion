...
export default function InventarioVerniciDettaglio({
  ...
  async function registraConteggio(entitaId: string) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/verniciatura/magazzino/inventario/${inventarioId}/righe/${entitaId}/conta`, {
        method: "POST",
  ...
  async function chiudi() {
    // Chiudibile in qualsiasi momento, anche con righe non contate — che restano alla giacenza
    // teorica di apertura (nessuna modifica silenziosa): l'utente va avvisato esplicitamente.
    const messaggio = daContareCount > 0
      ? `Ci sono ancora ${daContareCount} vernici non contate — resteranno alla giacenza teorica di apertura. Chiudere comunque l'inventario?`
    ...
    setError("");
    try {
      const res = await fetch(`/api/verniciatura/magazzino/inventario/${inventarioId}/chiudi`, { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.error ?? `Errore ${res.status}`);
      router.refresh();
    } catch (e) {
