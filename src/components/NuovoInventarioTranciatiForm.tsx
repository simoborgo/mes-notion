...
export default function NuovoInventarioTranciatiForm() {
  ...
  async function apri() {
    ...
    setError("");
    try {
      const res = await fetch("/api/magazzino/tranciati/inventario", { method: "POST" });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error ?? `Errore ${res.status}`);
      router.push(`/magazzino/tranciati/inventario/${data.id}`);
      router.refresh();
    } catch (e) {
  ...
  return (
    <div className="rounded-xl border-2 p-4 space-y-4" style={{ borderColor: "#e5e4e0", background: "white" }}>
      ...
      <p className="text-sm" style={{ color: "var(--color-grey-mid)" }}>
        Conteggio dell&apos;intero catalogo Tranciati attivi.
      </p>
