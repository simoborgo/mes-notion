...
export default function InventarioTranciatiDettaglio({
  ...
  async function registraConteggio(entitaId: string) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/magazzino/tranciati/inventario/${inventarioId}/righe/${entitaId}/conta`, {
        method: "POST",
  ...
  async function chiudi() {
    const messaggio = daContareCount > 0
      ? `Ci sono ancora ${daContareCount} tranciati non contati — resteranno alla giacenza teorica di apertura. Chiudere comunque l'inventario?`
    ...
    setError("");
    try {
      const res = await fetch(`/api/magazzino/tranciati/inventario/${inventarioId}/chiudi`, { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.error ?? `Errore ${res.status}`);
      router.refresh();
    } catch (e) {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--color-grey-mid)", background: "#faf9f7" }}>
              <th className="px-4 py-3">Essenza</th>
