...
export default function InventarioLegnoDettaglio({
  ...
  async function registraConteggio(entitaId: string) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/magazzino/legno/inventario/${inventarioId}/righe/${entitaId}/conta`, {
        method: "POST",
  ...
  async function chiudi() {
    const messaggio = daContareCount > 0
      ? `Ci sono ancora ${daContareCount} legni non contati — resteranno alla giacenza teorica di apertura. Chiudere comunque l'inventario?`
    ...
    setError("");
    try {
      const res = await fetch(`/api/magazzino/legno/inventario/${inventarioId}/chiudi`, { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.error ?? `Errore ${res.status}`);
      router.refresh();
    } catch (e) {
