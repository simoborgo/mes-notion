...
import type { Bordo, UnitaMisuraBordo } from "@/lib/types";
...
interface Props {
  bordo: Bordo | null;
  onClose: () => void;
  onSalvato: (b: Bordo) => void;
}

export default function BordoFormModal({ bordo, onClose, onSalvato }: Props) {
  const isEdit = !!bordo;
  const [codice, setCodice] = useState(bordo?.codice ?? "");
  const [decorCodice, setDecorCodice] = useState(bordo?.decorCodice ?? "");
  const [decorNome, setDecorNome] = useState(bordo?.decorNome ?? "");
  const [materiale, setMateriale] = useState(bordo?.materiale ?? "");
  const [spessoreMm, setSpessoreMm] = useState(bordo?.spessoreMm != null ? String(bordo.spessoreMm) : "");
  const [altezzaMm, setAltezzaMm] = useState(bordo?.altezzaMm != null ? String(bordo.altezzaMm) : "");
  const [finitura, setFinitura] = useState(bordo?.finitura ?? "");
  const [fornitore, setFornitore] = useState(bordo?.fornitore ?? "");
  const [codiceFornitore, setCodiceFornitore] = useState(bordo?.codiceFornitore ?? "");
  const [codiceInventario, setCodiceInventario] = useState(bordo?.codiceInventario ?? "");
  const [unitaMisura, setUnitaMisura] = useState<UnitaMisuraBordo | "">(bordo?.unitaMisura ?? "");
  const [clienteRiferimento, setClienteRiferimento] = useState(bordo?.clienteRiferimento ?? "");
  const [attivo, setAttivo] = useState(bordo?.attivo ?? true);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...
        decorCodice: decorCodice.trim() || null,
        decorNome: decorNome.trim() || null,
        materiale: materiale.trim() || null,
        spessoreMm: spessoreMm.trim() ? Number(spessoreMm) : null,
        altezzaMm: altezzaMm.trim() ? Number(altezzaMm) : null,
      ...
      const res = await fetch(isEdit ? `/api/magazzino/bordi/${bordo!.id}` : "/api/magazzino/bordi", {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onClose}>
      <div className="w-full max-w-2xl bg-white rounded-lg shadow-2xl overflow-y-auto max-h-[90vh]" style={{ borderRadius: "var(--radius-modal)" }} onClick={(e) => e.stopPropagation()}>
        <div className="px-6 py-4 border-b flex items-start justify-between" style={{ background: "linear-gradient(135deg, rgba(124,58,237,0.06), rgba(219,39,119,0.06))" }}>
          <div>
            <h2 className="font-semibold text-base">{isEdit ? "Modifica bordo" : "Nuovo bordo"}</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelCls} style={{ color: "var(--color-grey-mid)" }}>Codice decor</label>
              <input type="text" className={inputCls} value={decorCodice} onChange={(e) => setDecorCodice(e.target.value)} placeholder="es. H1180 ST10" />
            </div>
            <div>
              <label className={labelCls} style={{ color: "var(--color-grey-mid)" }}>Descrizione decor</label>
              <input type="text" className={inputCls} value={decorNome} onChange={(e) => setDecorNome(e.target.value)} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="text" className={inputCls} value={materiale} onChange={(e) => setMateriale(e.target.value)} placeholder="ABS, PVC, Melaminico…" />
            </div>
            <div>
            ...
            <div>
              <label className={labelCls} style={{ color: "var(--color-grey-mid)" }}>Altezza (mm)</label>
              <input type="number" min="0" step="0.1" className={inputCls} value={altezzaMm} onChange={(e) => setAltezzaMm(e.target.value)} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" className={inputCls} value={finitura} onChange={(e) => setFinitura(e.target.value)} placeholder="opaco, lucido, goffrato…" />
            </div>
            <div>
              <label className={labelCls} style={{ color: "var(--color-grey-mid)" }}>Unità di misura</label>
              <select className={inputCls} value={unitaMisura} onChange={(e) => setUnitaMisura(e.target.value as UnitaMisuraBordo | "")}>
                <option value="">—</option>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {saving ? "Salvataggio…" : isEdit ? "Salva modifiche" : "Crea bordo"}
            </button>
          </div>
        </form>
      </div>
