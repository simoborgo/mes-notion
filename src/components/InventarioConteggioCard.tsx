...
export default function InventarioConteggioCard({
  ...
  async function eseguiConta(q: number) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/ferramenta/inventario/${sessioneId}/righe/${articolo.id}/conta`, {
        method: "POST",
      ...
      <div className="rounded-xl border-2 p-4 space-y-2" style={{ borderColor: "#86EFAC", background: "#F0FDF4" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "#166534" }}>{articolo.descrizione} — contate {quantita} {articolo.unitaMisura}</p>
          </div>
        </div>
        {delta !== null && delta !== 0 && (
  ...
  return (
    <div className="rounded-xl border-2 p-4 space-y-3" style={{ borderColor: "#FCD34D", background: "#FFFBEB" }}>
      <div>
        ...
        <p className="font-bold text-lg mt-1" style={{ color: "var(--color-black)" }}>{articolo.descrizione}</p>
        <p className="text-sm" style={{ color: "var(--color-grey-mid)" }}>{articolo.codiceOs1}</p>
      </div>
      <p className="text-sm" style={{ color: "var(--color-black)" }}>
        Giacenza teorica: <strong>{riga.giacenzaTeorica} {articolo.unitaMisura}</strong>
      </p>

      <div>
        <label className="text-xs font-medium block mb-1" style={{ color: "var(--color-grey-mid)" }}>
          Quantità contata ({articolo.unitaMisura || "unità"})
        </label>
        <input
          type="number"
          min="0"
