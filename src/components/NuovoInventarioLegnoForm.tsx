...
export default function NuovoInventarioLegnoForm() {
  ...
  async function apri() {
    ...
    setError("");
    try {
      const res = await fetch("/api/magazzino/legno/inventario", { method: "POST" });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error ?? `Errore ${res.status}`);
      router.push(`/magazzino/legno/inventario/${data.id}`);
      router.refresh();
    } catch (e) {
  ...
  return (
    <div className="rounded-xl border-2 p-4 space-y-4" style={{ borderColor: "#e5e4e0", background: "white" }}>
      ...
      <p className="text-sm" style={{ color: "var(--color-grey-mid)" }}>
        Conteggio dell&apos;intero catalogo Legname attivo.
      </p>
