...
export default function ProfiloMetallicoCaricoScaricoModal({
  profilo, tipo, onClose, onSalvato,
}: {
  profilo: ProfiloMetallico;
  tipo: "carico" | "scarico";
  onClose: () => void;
  ...
  async function salva() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/magazzino/profili-metallici/${profilo.id}/${tipo}`, {
        method: "POST",
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.4)" }}>
      <div className="w-full max-w-sm rounded-2xl bg-white p-5 space-y-4 shadow-xl">
        <div>
          <h2 className="text-lg font-semibold" style={{ fontFamily: "var(--font-display)" }}>{titolo} — {profilo.tipoProfilo ?? profilo.codice ?? "Profilo"}</h2>
          <p className="text-xs mt-1" style={{ color: "var(--color-grey-mid)" }}>
            Giacenza attuale: {profilo.giacenzaAttuale} {profilo.unitaMisura ?? ""}
          </p>
        </div>
        <div>
          <label className="block text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: "var(--color-grey-mid)" }}>
            Quantità {profilo.unitaMisura ? `(${profilo.unitaMisura})` : ""}
          </label>
          <input
