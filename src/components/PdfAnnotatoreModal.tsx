...
export interface AnnotazioneData {
...
interface Props {
  ...
  schedaOdp: string;
  onClose: () => void;
  // Modalità "save": salva direttamente su Notion
  rilavorazionePageId?: string;
  onSaved?: () => void;
  // Modalità "callback": restituisce i dati al wizard
  onAnnotated?: (data: AnnotazioneData) => void;
}
...
export default function PdfAnnotatoreModal({ sourcePdfPageId, schedaOdp, onClose, rilavorazionePageId, onSaved, onAnnotated }: Props) {
  ...
  const redrawAnnotations = useCallback((liveStroke?: Point[]) => {
    ...
    for (const s of pageStamps) {
      const cx = s.x * W; const cy = s.y * H;
      const isOk = s.tipo === "ok";
      ctx.beginPath(); ctx.ellipse(cx, cy, r, r, 0, 0, 2 * Math.PI);
      ctx.fillStyle = isOk ? "#2E8B4F" : "#CC3333"; ctx.fill();
  ...
  const renderPage = useCallback(async (pageNum: number) => {
    ...
    const vp = (page as { getViewport: (o: { scale: number }) => { width: number; height: number } }).getViewport({ scale });
  ...
  const attachPointerHandlers = useCallback(() => {
    ...
    function onDown(e: PointerEvent) {
      ...
      const pt = norm(e); pointerStartRef.current = pt; isDrawingRef.current = false; currentStrokeRef.current = [pt];
      longPressTimerRef.current = setTimeout(() => {
        cancelLongPress();
        const p = pointerStartRef.current!; const page = currentPageRef.current;
    ...
    function onMove(e: PointerEvent) {
      ...
      const pt = norm(e); const start = pointerStartRef.current!;
      if (Math.hypot(pt.x - start.x, pt.y - start.y) > TAP_MOVE_THRESHOLD * 0.5) { cancelLongPress(); isDrawingRef.current = true; }
      currentStrokeRef.current.push(pt);
      if (isDrawingRef.current) redrawAnnotations(currentStrokeRef.current);
    }
    function onUp(e: PointerEvent) {
      e.preventDefault(); cancelLongPress();
      const stroke = currentStrokeRef.current; currentStrokeRef.current = [];
      if (stroke.length === 0) return;
      const start = pointerStartRef.current!; const end = norm(e);
      ...
      if (moved >= TAP_MOVE_THRESHOLD && isDrawingRef.current) {
        ...
        const page = currentPageRef.current;
        const nearIdx = (stampsRef.current[page] ?? []).findIndex(s => Math.hypot(s.x - end.x, s.y - end.y) < 0.05);
        if (nearIdx >= 0) {
          ...
          if (last && now - last.time < DOUBLE_TAP_MS && last.page === page && Math.hypot(end.x - last.x, end.y - last.y) < DOUBLE_TAP_DIST) {
            ...
            lastTapRef.current = null;
          } else { lastTapRef.current = { time: now, x: end.x, y: end.y, page }; }
        }
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        pdfDocRef.current = pdf; setTotalPages(pdf.numPages); setCurrentPage(1); currentPageRef.current = 1; setPdfLoading(false);
      } catch (e) { if (!cancelled) { setPdfError((e as Error).message); setPdfLoading(false); } }
    })();
    return () => { cancelled = true; };
  }, [sourcePdfPageId]);

  useEffect(() => { if (!pdfLoading && totalPages > 0) renderPage(currentPage); }, [currentPage, totalPages, pdfLoading, renderPage]);
  useEffect(() => { if (totalPages > 0) redrawAnnotations(); }, [strokes, stamps, totalPages, redrawAnnotations]);
  useEffect(() => { if (totalPages === 0) return; return attachPointerHandlers(); }, [totalPages, attachPointerHandlers]);
  useEffect(() => {
    let timer: ReturnType<typeof setTimeout>;
    function onResize() { clearTimeout(timer); timer = setTimeout(() => { if (pdfDocRef.current) renderPage(currentPageRef.current); }, 200); }
    window.addEventListener("resize", onResize);
    return () => window.removeEventListener("resize", onResize);
  }, [renderPage]);

  function undoLast() {
    setStrokes(prev => { const arr = [...(prev[currentPage] ?? [])]; arr.pop(); return { ...prev, [currentPage]: arr }; });
  }
  function clearPage() {
  ...
  const isCallbackMode = !!onAnnotated;
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col" style={{ background: "#1a1a1a" }}>
      <div className="shrink-0 flex items-center gap-3 px-4 py-2.5 border-b" style={{ background: "#242424", borderColor: "#333" }}>
        <button onClick={onClose} className="flex items-center justify-center w-8 h-8 rounded-full hover:bg-white/10 transition-colors" style={{ color: "#aaa" }}>✕</button>
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-white truncate">
            {isCallbackMode ? "Annota PDF — step 2 di 3" : `Annota PDF — ${schedaOdp}`}
          </div>
          <div className="text-xs" style={{ color: "#888" }}>Disegna · Tieni premuto = MANCA · Doppio tap = OK · Tap su bollo = rimuovi</div>
        </div>
        {totalPages > 1 && (
          <div className="flex items-center gap-2">
            <button disabled={currentPage <= 1} onClick={() => setCurrentPage(p => p - 1)} className="w-8 h-8 rounded flex items-center justify-center disabled:opacity-30 hover:bg-white/10 text-white text-lg">‹</button>
            <span className="text-xs text-white tabular-nums">{currentPage} / {totalPages}</span>
            <button disabled={currentPage >= totalPages} onClick={() => setCurrentPage(p => p + 1)} className="w-8 h-8 rounded flex items-center justify-center disabled:opacity-30 hover:bg-white/10 text-white text-lg">›</button>
          </div>
        )}
        <div className="flex items-center gap-2">
          <button onClick={undoLast} disabled={!(strokes[currentPage]?.length > 0)} className="px-2.5 py-1.5 rounded text-xs font-medium disabled:opacity-30 hover:bg-white/10" style={{ color: "#ccc", border: "1px solid #444" }}>↩ Annulla</button>
          <button onClick={clearPage} className="px-2.5 py-1.5 rounded text-xs font-medium hover:bg-white/10" style={{ color: "#ccc", border: "1px solid #444" }}>Pulisci</button>
          {isCallbackMode ? (
            <>
              <button onClick={() => onAnnotated({ strokes: {}, stamps: {} })}
                className="px-2.5 py-1.5 rounded text-xs font-medium hover:bg-white/10"
                style={{ color: "#ccc", border: "1px solid #444" }}>
                Salta →
              </button>
              <button onClick={() => onAnnotated({ strokes, stamps })}
                className="px-3 py-1.5 rounded text-sm font-semibold hover:opacity-90"
                style={{ background: "#D97706", color: "white" }}>
                Conferma annotazione →
              </button>
            </>
          ) : saved ? (
            <span className="px-3 py-1.5 rounded text-xs font-semibold" style={{ background: "#065F46", color: "#A7F3D0" }}>✓ Salvato</span>
          ) : (
            <button onClick={handleSave} disabled={saving || !hasAnnotations} className="px-3 py-1.5 rounded text-sm font-semibold disabled:opacity-50 hover:opacity-90" style={{ background: "#D97706", color: "white" }}>
