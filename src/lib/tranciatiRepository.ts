...
import { pool } from "./db";
import type { Tranciato, TranciatoUpdate } from "./types";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function mapRow(r: any): Tranciato {
  return {
    id: r.id,
    codice: r.codice,
    essenza: r.essenza,
...
export interface TranciatiFiltro {
  soloAttivi?: boolean;
  essenza?: string;
  fornitore?: string;
...
export async function getTranciati(filtro: TranciatiFiltro = {}): Promise<Tranciato[]> {
  const where: string[] = [];
  ...
  const { rows } = await pool.query(
    `SELECT * FROM tranciati ${whereClause} ORDER BY essenza NULLS LAST, codice`,
    values
  );
  return rows.map(mapRow);
}

export async function getTranciatoById(id: string): Promise<Tranciato> {
  const { rows } = await pool.query(`SELECT * FROM tranciati WHERE id = $1`, [id]);
  if (rows.length === 0) throw new Error(`Tranciato non trovato: ${id}`);
  return mapRow(rows[0]);
}

export async function createTranciato(data: {
  codice?: string | null;
  essenza?: string | null;
  qualita?: string | null;
...
export async function updateTranciato(id: string, data: TranciatoUpdate & { updatedBy?: string | null }): Promise<Tranciato> {
  const sets: string[] = [];
  const values: unknown[] = [];
  let i = 1;

  const campi: [keyof TranciatoUpdate, string][] = [
    ["codice", "codice"],
    ["essenza", "essenza"],
    ["qualita", "qualita"],
  ...
  values.push(id);
  const { rows } = await pool.query(
    `UPDATE tranciati SET ${sets.join(", ")} WHERE id = $${i} RETURNING *`,
    values
  );
  if (rows.length === 0) throw new Error(`Tranciato non trovato: ${id}`);
  return mapRow(rows[0]);
...
export async function disattivaTranciato(id: string): Promise<void> {
  await updateTranciato(id, { attivo: false });
}

export async function aggiornaGiacenzaTranciato(id: string, giacenzaAttuale: number, executor: Pool | PoolClient = pool): Promise<void> {
  await executor.query(`UPDATE tranciati SET giacenza_attuale = $1, updated_at = now() WHERE id = $2`, [giacenzaAttuale, id]);
}

export async function segnalaMovimentoTranciato(id: string, executor: Pool | PoolClient = pool): Promise<void> {
  await executor.query(`UPDATE tranciati SET segnalata_uso_il = now() WHERE id = $1`, [id]);
}

export async function risolviSegnalazioneTranciato(id: string, executor: Pool | PoolClient = pool): Promise<void> {
  await executor.query(`UPDATE tranciati SET segnalata_uso_il = NULL WHERE id = $1`, [id]);
}
